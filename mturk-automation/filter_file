...
def options():
    ...
    Checkbutton(root, bg='white', text='Do You Want to Distribute by Source?', variable=source).pack()

    return test_percent, shuff_num, source
...
def check_box(df):
    ...
    for selector in columns[2:]:
        if 'severity' in selector or (selector in labels) or ('Votes' in selector) or 'second' in selector:
            continue
        grand_dict[selector] = {}
